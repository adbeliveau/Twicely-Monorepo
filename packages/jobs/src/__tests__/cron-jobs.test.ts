...
describe('cron-jobs', () => {
  ...
  it('registerCronJobs registers all platform and external cron jobs', async () => {
    ...
    // 7 platform cron jobs (incl. listing image retention, Decision #111) + 1 tax document + 1 affiliate suspension expiry + 4 cleanup queue jobs (G8) + 1 helpdesk retention purge (G9.6) + 1 helpdesk auto-close + 1 helpdesk SLA check + 1 helpdesk CSAT send
