...
export async function createLocalTransactionLedgerEntries(
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(ledgerEntry).values({
      ...
      idempotencyKey: `local:${orderId}:capture`,
    ...
    await tx.insert(ledgerEntry).values({
      ...
      idempotencyKey: `local:${orderId}:fee`,
    ...
    await tx.insert(ledgerEntry).values({
      ...
      idempotencyKey: `local:${orderId}:stripe_fee`,
...
export async function createPriceAdjustmentLedgerEntry(
  ...
  await db.insert(ledgerEntry).values({
    ...
    idempotencyKey: `local:${orderId}:price_adjustment`,
...
export async function createLocalCancelRefundLedgerEntry(
  ...
  await db.insert(ledgerEntry).values({
    ...
    idempotencyKey: `local:${orderId}:cancel_refund`,
