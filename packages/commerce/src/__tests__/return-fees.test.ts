...
describe('Return Fees Module', () => {
  ...
  describe('calculateRestockingFee', () => {
    ...
    it('respects minimum of $1 for small orders', async () => {
      const { calculateRestockingFee, RESTOCKING_FEE_MIN_CENTS } = await import('@/lib/commerce/return-fees');
    ...
    it('respects maximum of $50 for large orders', async () => {
      const { calculateRestockingFee, RESTOCKING_FEE_MAX_CENTS } = await import('@/lib/commerce/return-fees');
