...
import { HubNotificationDropdown } from "@/components/header/HubNotificationDropdown";
import { HubUserDropdown } from "@/components/header/HubUserDropdown";
...
const AppHeader: React.FC<AppHeaderProps> = ({ displayName, email, roles }) => {
  ...
  return (
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 xl:h-[72px] xl:border-b">
      <div className="flex flex-col items-center justify-between grow xl:h-[72px] xl:flex-row xl:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 xl:h-full xl:justify-normal xl:border-b-0 xl:px-0 xl:py-0">
          <button
            className={`flex items-center justify-center w-10 h-10 text-gray-500 border-gray-200 rounded-lg z-99999 dark:border-gray-800 dark:text-gray-400 lg:h-11 lg:w-11 xl:border ${
          ...
          <Link href="/d" className="xl:hidden">
          ...
          <div className="hidden xl:block">
            <div className="relative">
              <span className="absolute -translate-y-1/2 left-4 top-1/2 pointer-events-none">
                <svg className="fill-gray-500 dark:fill-gray-400" width="20" height="20" viewBox="0 0 20 20"><path fillRule="evenodd" clipRule="evenodd" d="M3.04175 9.37363C3.04175 5.87693 5.87711 3.04199 9.37508 3.04199C12.8731 3.04199 15.7084 5.87693 15.7084 9.37363C15.7084 12.8703 12.8731 15.7053 9.37508 15.7053C5.87711 15.7053 3.04175 12.8703 3.04175 9.37363ZM9.37508 1.54199C5.04902 1.54199 1.54175 5.04817 1.54175 9.37363C1.54175 13.6991 5.04902 17.2053 9.37508 17.2053C11.2674 17.2053 13.003 16.5344 14.357 15.4176L17.177 18.238C17.4699 18.5309 17.9448 18.5309 18.2377 18.238C18.5306 17.9451 18.5306 17.4703 18.2377 17.1774L15.418 14.3573C16.5365 13.0033 17.2084 11.2669 17.2084 9.37363C17.2084 5.04817 13.7011 1.54199 9.37508 1.54199Z" fill=""/></svg>
              </span>
              <input
                ref={inputRef}
                type="text"
              ...
              />
              <button type="button" className="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[4.5px] text-xs text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
                <span>&#8984;</span><span>K</span>
              </button>
            </div>
          </div>
        </div>
        <div className={`${isApplicationMenuOpen ? "flex" : "hidden"} items-center justify-between w-full gap-4 px-5 py-4 xl:flex shadow-theme-md xl:h-full xl:justify-end xl:px-0 xl:py-0 xl:shadow-none`}>
          <div className="flex items-center gap-2 2xsm:gap-3">
            <ThemeToggleButton />
            <HubNotificationDropdown />
          </div>
          <div className="flex items-center gap-3">
            {displayName && email && roles ? (
              <HubUserDropdown displayName={displayName} email={email} roles={roles} />
            ) : (
