import { Html, Head, Body, Container, Heading, Text, Link, Hr } from '@react-email/components';

interface QaAnswerReceivedEmailProps {
  recipientName: string;
  itemTitle: string;
  answerText: string;
  listingUrl: string;
}

export default function QaAnswerReceivedEmail({
  recipientName,
  itemTitle,
  answerText,
  listingUrl,
}: QaAnswerReceivedEmailProps) {
  return (
    <Html>
      <Head />
      <Body style={{ fontFamily: 'sans-serif', background: '#f9f9f9', padding: 20 }}>
        <Container style={{ maxWidth: 480, margin: '0 auto', background: '#fff', padding: 24, borderRadius: 8 }}>
          <div style={{ height: 4, background: '#7C3AED', borderRadius: '8px 8px 0 0', marginBottom: 16 }} />
          <Heading as="h2" style={{ margin: '0 0 16px' }}>Your Question Was Answered</Heading>
          <Text style={{ margin: '0 0 12px' }}>Hi {recipientName},</Text>
          <Text style={{ margin: '0 0 12px' }}>
            The seller answered your question on <strong>{itemTitle}</strong>:
          </Text>
          <Text style={{ margin: '0 0 16px', fontStyle: 'italic', color: '#444' }}>
            &ldquo;{answerText}&rdquo;
          </Text>
          <Text style={{ margin: '0 0 16px' }}>
            <Link href={listingUrl} style={{ color: '#7C3AED' }}>View listing →</Link>
          </Text>
          <Hr style={{ borderColor: '#eee', margin: '20px 0' }} />
          <Text style={{ fontSize: 12, color: '#666', margin: 0 }}>
            You received this because a seller answered your question on Twicely.
          </Text>
        </Container>
      </Body>
    </Html>
  );
}
