...
import { updateIntegrationKeys, testStripeConnection, toggleIntegrationModule } from '@/lib/actions/admin-integrations';
...
export function StripeSettingsForm({ settings, canEdit }: StripeSettingsFormProps) {
  ...
  function handleToggle() {
    ...
    startTransition(async () => {
      const result = await toggleIntegrationModule({ moduleKey: 'integrations.stripe.enabled', enabled: next });
      if (!result.success) {
        setEnabled(!next);
        toast.error(result.error ?? 'Failed to update Stripe module');
      }
    });
  }
  ...
  function handleSaveKeys() {
    startTransition(async () => {
      const payload: Record<string, string> = { provider: 'stripe' };
      if (testKey) payload.testSecretKey = testKey;
      if (liveKey) payload.liveSecretKey = liveKey;
      if (testPub) payload.testPublishableKey = testPub;
      if (livePub) payload.livePublishableKey = livePub;
  ...
  function handleTestConnection() {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error(result.message ?? 'Stripe connection test failed');
      }
    });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-white p-4">
        <div>
          <h3 className="text-sm font-medium text-gray-900">Stripe Payments Module</h3>
          <p className="text-xs text-gray-500">Enable Stripe for payment processing and Connect payouts</p>
        </div>
        <button
          type="button"
          role="switch"
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <input
              type="password"
              placeholder={settings.hasTestSecretKey ? '••••••••••••••••' : 'sk_test_...'}
              value={testKey}
          ...
          <div>
            ...
            <input
              type="password"
              placeholder={settings.hasLiveSecretKey ? '••••••••••••••••' : 'sk_live_...'}
              value={liveKey}
          ...
          <div>
            ...
            <input
              type="text"
              placeholder={settings.hasTestPublishableKey ? '••••••••••••••••' : 'pk_test_...'}
              value={testPub}
              onChange={(e) => setTestPub(e.target.value)}
              disabled={!canEdit || isPending}
              className="w-full rounded-md border border-gray-200 px-3 py-1.5 text-sm disabled:opacity-50"
            />
          </div>
          <div>
            ...
            <input
              type="text"
              placeholder={settings.hasLivePublishableKey ? '••••••••••••••••' : 'pk_live_...'}
              value={livePub}
              onChange={(e) => setLivePub(e.target.value)}
              disabled={!canEdit || isPending}
              className="w-full rounded-md border border-gray-200 px-3 py-1.5 text-sm disabled:opacity-50"
            />
          </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={!canEdit || isPending || (!testKey && !liveKey && !testPub && !livePub)}
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-3">
        ...
        <div>
          ...
          <code className="block rounded-md bg-gray-50 px-3 py-2 text-xs text-gray-800">
            {settings.webhookUrl ?? 'https://twicely.co/api/webhooks/stripe'}
          </code>
        </div>
        <div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        ...
        <p className="mt-1 text-xs text-gray-500">
          {settings.connectEnabled
            ? 'Connect is enabled for seller payouts'
            : 'Connect is not yet enabled — configure in Stripe Dashboard'}
        </p>
      </div>
    </div>
  );
