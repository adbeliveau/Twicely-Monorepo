/**
 * Whatnot OAuth callback route — H2.1
 * GET /api/crosslister/whatnot/callback
 *
 * Handles the OAuth authorization code exchange after the seller
 * approves access on Whatnot's authorization page.
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (!code) {
    logger.warn('[whatnot/callback] Missing authorization code');
  ...
  try {
    ...
    if (!authResult.success || !authResult.accessToken) {
      logger.error('[whatnot/callback] Auth failed', { sellerId, error: authResult.error });
      return NextResponse.redirect(new URL(CONNECT_FAILURE_URL, request.url));
    }
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(crosslisterAccount.channel, 'WHATNOT'),
        ),
      )
      .limit(1);
    ...
    if (existing) {
      ...
      // TODO: Encrypt tokens before production (stored as plain text for H2.1)
    ...
    logger.error('[whatnot/callback] Unexpected error', { sellerId, error: String(err) });
    return NextResponse.redirect(new URL(CONNECT_FAILURE_URL, request.url));
  }
}
