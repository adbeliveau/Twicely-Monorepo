/**
 * QuickBooks OAuth authorization start — G10.3
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const enabled = await getPlatformSetting<boolean>('accounting.quickbooks.enabled', false);
  if (!enabled) {
    logger.warn('[quickbooks/authorize] QuickBooks integration not enabled');
    return NextResponse.redirect(new URL(CONNECT_FAILURE_URL, request.url));
  }
  ...
  try {
    ...
    cookieStore.set('qb_oauth_state', JSON.stringify({ state, userId }), {
      httpOnly: true,
    ...
    logger.error('[quickbooks/authorize] Failed to build auth URL', { userId, error: String(err) });
    return NextResponse.redirect(new URL(CONNECT_FAILURE_URL, request.url));
  }
}
